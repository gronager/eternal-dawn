...
- **NOT technicolor, and the sextet is DEAD.** The strong IR sector is the torsion-bound soliton of Eq. 6.2 (Chapter 6, `sims/.../self_consistent.py`), *not* a sextet gauge theory. The SU(3) N_f=2 two-index-symmetric ("sextet") dynamical-fermion runs were a discarded lattice proxy and a wrong turn — do **not** resurrect `generate_sextet`, `--rep sextet`, or any γ_m-gate framing as the program. The framework only *resembles* technicolor in that the walking sector needs an electroweak `S < 0.1`; everything else (40 years of technicolor model-building) is explicitly not our path. The lattice targets are L1–L5 in Appendix B: L1 confinement (dual superconductor, linear `V=σL`), L2 the sharp prediction `σ = 2πv²` (one substrate sets both mass and confinement), L4 the soliton overlap masses, L5 the propagating-torsion β-function — none of which is a sextet γ_m.